...
describe("Products API", () => {
  ...
  describe("GET /api/products", () => {
    test("should return all products with valid query parameters", async () => {
      const res = await request(app)
        .get("/api/products?search=Test")
        .set("Authorization", `Bearer ${token}`);

      expect(res.status).toBe(200);
      expect(res.body.data).toHaveLength(1); // Assuming 1 product matches
      expect(res.body.data[0].name).toBe(sampleProduct.name);
    });

    test("should handle invalid search parameter gracefully", async () => {
      const res = await request(app)
        .get("/api/products?search=%^&")
        .set("Authorization", `Bearer ${token}`);

      expect(res.status).toBe(200);
      expect(res.body.data).toHaveLength(0); // No products match
    });
    ...
    test("should return 400 for invalid sort parameter", async () => {
      ...
      expect(res.status).toBe(400); // Should return error for invalid sort
    });
  });
  ...
  describe("POST /api/products", () => {
    ...
    test("should return 400 if product with same SKU already exists", async () => {
      const newProduct = {
        ...sampleProduct,
        sku: "TEST123", // Use an existing SKU
      };
  ...
  describe("PUT /api/products/:id", () => {
    ...
    test("should return 400 if SKU is duplicated when updating product", async () => {
      const otherProduct = new Product({ ...sampleProduct, sku: "TEST456" });
      await otherProduct.save();

      const updatedProduct = { ...sampleProduct, sku: "TEST456" }; // Duplicate SKU
  ...
  describe("PATCH /api/products/:id/stock", () => {
    test("should increase stock quantity", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.stockQuantity).toBe(
        sampleProduct.stockQuantity + adjustment.adjustment
      );
    });
    ...
    test("should return 400 if insufficient stock for decrease", async () => {
      const adjustment = { adjustment: -200, reason: "Sales" }; // Trying to reduce more than available stock
    ...
    test("should return 400 for invalid stock adjustment value", async () => {
      const adjustment = { adjustment: "invalid", reason: "Invalid input" };
      ...
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Invalid adjustment data");
    });
